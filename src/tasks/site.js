...
function makeEntryStream(objects, project, placeholder) {
	...
	for (let objName in objects) {
		...
		for (let path of objects[objName]) {
			...
			rs.push(`exports[${JSON.stringify(objName)}][${nameEsc}]`)
			rs.push(` = require(${pathEsc})\n`)
		}
	}
	...
	for (let moduleMount in project.modules) {
		let moduleUri = JSON.stringify(project.modules[moduleMount] || moduleMount)
		rs.push(`exports.modules[${JSON.stringify(moduleMount)}]`)
		rs.push(` = function importModule() { return ${placeholder}(${moduleUri}) }\n`)
	}
